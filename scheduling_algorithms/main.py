...
def sjn():
    ...
    def sjf_scheduling(processes):
        n = len(processes)
        processes.sort(key=lambda x: x[1])  # sort by arrival time first
        ...
        while len(completed) < n:
            ...
            for p in processes:
                if p not in completed and p not in ready_queue and p[1] <= time:
        ...
        combined = []
        for i, (pid, at, bt) in enumerate(completed):
            combined.append((pid, at, bt, FT[i], WT[i], RT[i], TAT[i]))
        ...
        print(f"Average Waiting Time (AWT) = sum(WT) / #process = \t\t{sum(WT)} / {n} = {AWT:.2f}")
        print(f"CPU Utilization = (TBT / TFT)* 100 = \t\t\t\t({TBT} / {TFT}) * 100 = {CPU_UTIL:.2f}%")
        print(f"Throughput = #process / TBT = \t\t\t\t\t({n} / {TBT}) * 100 = {THROUGHPUT:.4f} or {THP:.2f}%")
        print("="*100)

        # Draw Gantt chart
        root = tk.Tk()
        root.title("SJF Scheduling - Gantt Chart")
        try:
            root.iconbitmap("assets/icon.ico")
        except:
            pass 
        ...
        for task in gantt_chart:
            ...
            x1 = x_start + start * scale
            x2 = x_start + end * scale
            canvas.create_rectangle(x1, 30, x2, 70, fill="lightgreen", outline="black")
            canvas.create_text((x1 + x2) // 2, 50, text=label)
            canvas.create_text(x1, 75, text=str(start))
        canvas.create_text(x2, 75, text=str(end))

        threading.Thread(target=show_summary_table, args=(completed, FT, WT, RT, TAT, AWT, ATAT,)).start()
        root.mainloop()

    def show_summary_table(processes, FT, WT, RT, TAT, AWT, ATAT):
        root = tk.Toplevel()
        root.title("SJN/SJF Summary Table")
        try:
            root.iconbitmap("assets/icon.ico")
        except:
            pass 
    ...
    def show_process_table(processes):
        root = tk.Tk()
        root.title("Process Queue (SJN/SJF)")
        try:
            root.iconbitmap("assets/icon.ico")
        except:
            pass 
        ...
        processes_sorted = sorted(processes, key=lambda x: x[1])  # sort by arrival time
...
███████║╚█████╔╝██║         ██╔╝       ███████║╚█████╔╝██║ ╚████║
╚══════╝ ╚════╝ ╚═╝         ╚═╝        ╚══════╝ ╚════╝ ╚═╝  ╚═══╝                                              
Shortest Job First (SJF) / Shortest Job Next (SJN)  Scheduling Algorithm       
                    September 05, 2025                          
                    Code by Paul Mendoza                          
                            v1.0                                   
    """)
        threading.Thread(target=sjf_scheduling, args=(processes,)).start()
        threading.Thread(target=show_process_table, args=(processes,)).start()
...
def main():
    clear()
    while True:
        print("\n--- SCHEDULING ALGORITMS v1.1---")
        print("Options:")
        print("[1] First Come First Serve (FCFS)")
        print("[2] Shortest Job First or Shortest Job Next (SJF/SJN)")
        print("[3] Priority Scheduling (PS)")
        print("[4] Shortest Remaining Time (SRT)")
        print("[5] Round Robin Scheduling (RRS)")
        print("[6] Multilevel Queue Scheduling (MQS)")
        print("[q] Quit")
        command = input("\nSelect Option: ")
